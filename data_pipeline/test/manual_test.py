...
async def test_workflow():
    async with httpx.AsyncClient() as client:
        ...
        # Find an attack drone
        attack_drones = state.get('attack_data', [])
        if not attack_drones:
            print("No attack drones found in state.")
            # Print state for debugging
            # print(state)
            return
        
        drone_id = attack_drones[0]['drone_id']
        print(f"Using drone: {drone_id}")

        print("\n--- 3. Testing Optimistic Update on Engage ---")
        drone = next((d for d in attack_drones if d['drone_id'] == drone_id), None)
        if not drone or not drone.get('assigned_target_id'):
             print(f"Drone {drone_id} not assigned yet, deploying manually to {target_id}...")
