...
async def run_mission_flow_tests(client):
    ...
    for i in range(40):
        ...
        active_attackers = [d for d in assigned_drones if d.get('role') == 'attack' and d.get('flight_status') in ['ACTIVE', 'ATTACKING']]
        ...
        for d in assigned_drones:
            ...
            print(f"        {status_icon} {role_tag} {d['drone_id']} | סטטוס: {d['flight_status']:9} | מרחק: {dist:6.1f} מ' | תחמושת: {d.get('weapons_count', 0)}")
