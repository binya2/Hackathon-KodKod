...
from data_pipeline.attack_brain.kafka_service import process_deployment_stream
...
async def main():
    ...
    deploy_consumer = await get_kafka_consumer(['commands.deployment'], bootstrap_servers=bootstrap, group_id='attack_deploy', offset_reset='earliest')
    ...
    try:
        await asyncio.gather(process_deployment_stream(deploy_consumer, producer, RUNNING_IN_K8S), assignment_loop(producer))
    finally:
