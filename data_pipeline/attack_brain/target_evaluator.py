...
async def request_replacement(target: TargetTelemetry, producer: AIOKafkaProducer):
    deploy_cmd = {'action': 'DEPLOY_DRONE', 'role': 'attack', 'target_id': target.target_id,
...
async def evaluate_target_needs(target: TargetTelemetry, all_drones: List[DroneTelemetry], producer: AIOKafkaProducer):
    if target.target_id == 'TGT-INIT':
        return
    assigned_count = sum((1 for d in all_drones if
                          d.assigned_target_id == target.target_id and d.flight_status in ['ACTIVE', 'ATTACKING',
                                                                                           'EN_ROUTE']))
    if assigned_count < 2:
...
def find_new_target(active_targets: List[TargetTelemetry],
                    all_drones: List[DroneTelemetry]) -> Optional[TargetTelemetry]:
    return next((t for t in active_targets if sum((1 for d in all_drones if d.assigned_target_id == t.target_id)) < 2),
                None)
