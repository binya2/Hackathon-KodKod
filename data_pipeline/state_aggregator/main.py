...
if __name__ == '__main__':
    mock_state = WorldState(timestamp=datetime.now(timezone.utc), target_data=[TargetTelemetry(target_id='TGT_001', target_type=TargetType.VEHICLE.value, position=GeoPoint(lat=32.1, lon=34.8), confidence=0.99)], recon_data=[DroneTelemetry(drone_id='RECON_1', role=DroneRole.RECON.value, position=GeoPoint(lat=32.11, lon=34.81, alt=150.0), velocity=10.0, heading=0.0, battery_percent=90.0, flight_status='ACTIVE')])
