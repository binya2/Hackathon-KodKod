...
async def _validate_recon_for_engage(state: dict, target_id: str):
    ...
    for d in recons:
        if d.get('assigned_target_id') == target_id:
            if d.get('flight_status') == 'ACTIVE':
                recon_ready = True
                break
    if not recon_ready:
        await log_to_kafka('WARN', f'Blocked engage on {target_id} - Recon not ready.')
        raise HTTPException(status_code=400,
                            detail="Forbidden: Cannot engage. Recon drone is not in ACTIVE status at the target.")


def _validate_attack_drone_for_engage(state: dict, target_id: str, drone_id: str):
    drone = next((d for d in state.get('recon_data', []) + state.get('attack_data', []) if d['drone_id'] == drone_id),
                 None)
    if not drone:
...
async def execute_engage(drone_id: str, target_id: str):
    ...
    payload = {'drone_id': drone_id, 'action': 'EXECUTE_STRIKE', 'target_id': target_id, 'position': target['position'],
...
async def execute_drone_deployment(role: str, target_id: str):
    ...
    payload = {'action': 'DEPLOY_DRONE', 'role': role, 'target_id': target_id, 'position': target['position'],
               'timestamp': iso8601_utc_now()}
...
async def spawn_target_with_swarm(lat: float, lon: float):
    ...
    intel_payload = {'action': 'SPAWN_TARGET', 'target_id': target_id, 'lat': lat, 'lon': lon,
                     'timestamp': iso8601_utc_now()}
...
async def _deploy_initial_swarm(target_id: str, lat: float, lon: float):
    ...
    for dep in deployments:
        payload = {'action': 'DEPLOY_DRONE', 'role': dep['role'], 'target_id': target_id,
                   'position': {'lat': lat, 'lon': lon}, 'timestamp': iso8601_utc_now()}
        await produce_message(TOPIC_DEPLOYMENT, str(uuid.uuid4()), payload)
