...
from data_pipeline.attack_commander.kafka_client import produce_message, log_to_kafka, iso8601_utc_now
...
async def _fetch_current_state():
    ...
    return {'target_data': target_data, 'recon_data': recon_data, 'attack_data': attack_data}
...
def _validate_attack_drone_for_engage(state: dict, target_id: str, drone_id: str):
    drone = next((d for d in state.get('recon_data', []) + state.get('attack_data', []) if d['drone_id'] == drone_id), None)
    if not drone:
...
async def execute_engage(drone_id: str, target_id: str):
    ...
    payload = {'drone_id': drone_id, 'action': 'EXECUTE_STRIKE', 'target_id': target_id, 'position': target['position'], 'timestamp': iso8601_utc_now()}
...
async def execute_drone_deployment(role: str, target_id: str):
    ...
    payload = {'action': 'DEPLOY_DRONE', 'role': role, 'target_id': target_id, 'timestamp': iso8601_utc_now()}
...
async def spawn_target_with_swarm(lat: float, lon: float):
    ...
    intel_payload = {'action': 'SPAWN_TARGET', 'target_id': target_id, 'lat': lat, 'lon': lon, 'timestamp': iso8601_utc_now()}
...
async def execute_recall(drone_id: str):
    ...
    if not drone:
        raise HTTPException(status_code=404, detail=f'Drone {drone_id} not found.')
    payload = {'drone_id': drone_id, 'action': 'RECALL_DRONE', 'timestamp': iso8601_utc_now()}
    await produce_message(TOPIC_DRONES, drone_id, payload)
    return payload
...
async def execute_manual_move(drone_id: str, lat: float, lon: float, alt: float):
    ...
    if not drone:
        raise HTTPException(status_code=404, detail=f'Drone {drone_id} not found.')
    payload = {'drone_id': drone_id, 'action': 'MANUAL_MOVE', 'position': {'lat': lat, 'lon': lon, 'alt': alt}, 'timestamp': iso8601_utc_now()}
    await produce_message(TOPIC_DRONES, drone_id, payload)
    return payload
...
async def execute_resume_auto(drone_id: str):
    ...
    if not drone:
        raise HTTPException(status_code=404, detail=f'Drone {drone_id} not found.')
    payload = {'drone_id': drone_id, 'action': 'RESUME_AUTO', 'timestamp': iso8601_utc_now()}
    await produce_message(TOPIC_DRONES, drone_id, payload)
    return payload
...
async def _deploy_initial_swarm(target_id: str, lat: float, lon: float):
    ...
    for dep in deployments:
        payload = {'action': 'DEPLOY_DRONE', 'role': dep['role'], 'target_id': target_id, 'position': {'lat': lat, 'lon': lon}, 'timestamp': iso8601_utc_now()}
        await produce_message(TOPIC_DEPLOYMENT, str(uuid.uuid4()), payload)