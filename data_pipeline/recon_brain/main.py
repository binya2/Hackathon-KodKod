...
async def run_recon_brain():
    ...
    target_consumer = await get_kafka_consumer(['target.raw'], bootstrap_servers=bootstrap_servers, group_id='recon_brain_group', offset_reset='earliest')
    deploy_consumer = await get_kafka_consumer(['commands.deployment'], bootstrap_servers=bootstrap_servers, group_id='recon_deploy', offset_reset='earliest')
    await producer.start()
    await target_consumer.start()
    await deploy_consumer.start()
    logger.info('Recon Brain service started.')
    try:
        await asyncio.gather(process_target_stream(target_consumer, producer), process_deployment_stream(deploy_consumer, producer, RUNNING_IN_K8S))
    finally:
