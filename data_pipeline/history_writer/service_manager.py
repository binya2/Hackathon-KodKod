...
try:
    from kafka.consumer import create_history_consumer, poll_and_decode_messages
...
async def run_history_ingestion():
    ...
    try:
        while True:
            message = await asyncio.to_thread(poll_and_decode_messages, consumer, timeout=0.1)
            if message:
...
async def process_world_state(state: dict, collection):
    ...
    for d in state.get('recon_data', []):
        doc = {'drone_id': d.get('drone_id'), 'assigned_target_id': d.get('assigned_target_id'),
               'position': d.get('position'), 'timestamp': d.get('timestamp')}
